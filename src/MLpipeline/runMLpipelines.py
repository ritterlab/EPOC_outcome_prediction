...
def main():
    """
    Usage:
        python runMLpipelines.py <config name> [-d | --debug]

    Arguments:
        <config>   Specify the name of the configuration to use (required).
        The configuration file must be located in the `src/pipeline_configs` directory.
        Please copy one of the existing configurations as a starting point.

    Options:
        -d, --debug   Run the script in debug mode (optional).

    Example:
        python script_name.py my_config.json --debug
    """
    ...
    name = option.config
    run_debug = option.debug

    run_pipeline(name, run_debug)


def run_pipeline(name, run_debug = False):
    """
    Executes a machine learning pipeline by configuring and running experiments over
    input-output combinations, models, and confound control techniques.

    :param name: Name of the pipeline to execute. Must match the name of a module in the configuration directory `src.pipeline_configs`.
    :type name: str
    :param run_debug: A flag that indicates whether the function should run in debug mode.
    :type run_debug: bool
    :return: None
    :rtype: None
    """
    pkg = importlib.import_module(f'src.pipeline_configs.{name}')

    cfg = pkg.Config
    cfg_path = os.path.abspath(sys.modules[cfg.__module__].__file__)
    # set the output directory
    OUTPUT_DIR = '%s/%s' % cfg.OUTPUT_BASE_PATH,cfg_path.replace('config', 'results').replace('.py', '')
    ...
    with Parallel(n_jobs=cfg.N_JOBS) as parallel:
        for h5_file in cfg.H5_FILES:
            ...
            with h5py.File(h5_file, 'r') as data:
                ...
                for nreps in range(
                        cfg.N_REPEATS):  # edit Marija, run pipeline n times for better reliability due to small sample size
                    ...
                    for key, cfg_i in cfg.ANALYSIS.items():
                        ...
                        for conf_ctrl_tech in cfg_i["CONF_CTRL_TECHS"]:
                            for io in io_combinations:
                                for model_pipegrid in cfg_i[
                                    "MODEL_PIPEGRIDS"]:  # pipe=model_pipeline, grid=hyperparameters
                                    ...
                                    if conf_ctrl_tech == "loso":
                                        ...
                                        test_idxs = [test_idx for _, test_idx in
                                                     splitter.split(data["X"], groups=data['site'])]
                                    else:
                                        splitter = StratifiedKFold(n_splits=cfg.N_OUTER_CV, shuffle=True,
                                                                   random_state=seeds[
                                                                       nreps])  # edit Marija, different seed for each of the n runs
                                    ...
                                    if conf_ctrl_tech == 'baseline-cb': conf_run_names = ['sex',
                                                                                          'site']  # if 'baseline-cb' then control only for 'sex' and 'site' not for any additional variable/s given
                                    ...
                                    for trial in range(cfg.N_OUTER_CV):
                                        settings.extend([{"label_name": y, "conf_ctrl_tech": conf_ctrl_tech,
                                                          ...
                                                          "scoring_list": cfg_i["METRICS"],
                                                          "task_type": cfg_i["TASK_TYPE"],
                                                          "test_idx": test_idxs[trial],
                                                          "run_reps": nreps}])  # edit Marija, to avoid overwriting csv files due to identical name
            ...
            for i, setting in enumerate(settings):
                ...
                print(
                    f' ({i:2d}) {str(setting["conf_ctrl_tech"]) + ": " + ",".join(setting["confs"]):25s}{str(setting["io"]):20s}\
            ...
            parallel(delayed(
                ...
                save_models=cfg.SAVE_MODELS, debug=run_debug, random_state=seeds_inner[rnd])
